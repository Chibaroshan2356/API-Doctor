...
const DualAxisLineChart: React.FC<DualAxisLineChartProps> = ({
  ...
  const CustomTooltip = ({ active, payload, label }: TooltipProps<number, string>) => {
    if (active && payload && payload.length) {
      ...
      const responseTime = payload.find(p => p.dataKey === 'responseTime')?.value || 0;
      const errorRate = payload.find(p => p.dataKey === 'errorRate')?.value || 0;
      const errorCount = data.errorCount || Math.round((errorRate / 100) * 1000); // Estimate
          ...
          <div className="space-y-2">
            <div className="flex items-center space-x-2">
              <div 
                ...
                style={{ backgroundColor: colors.responseTime }}
              />
            ...
            <div className="flex items-center space-x-2">
              <div 
                className="w-3 h-3 rounded-full"
                style={{ backgroundColor: colors.errorRate }}
              />
              <span className="text-sm text-gray-600 dark:text-gray-400">
                Error Rate:
              </span>
